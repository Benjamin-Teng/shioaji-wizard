...
`Sinopac.pfx`）。若目前路徑沒有憑證且在 eLeader 預設位置
`C:\ekey\551\(身分證字號)\S` 只找到一個 `.pfx`，會自動帶入並儲存該路徑；找到
多個時才列出遮罩後的路徑，讓使用者按「使用這個」選擇。只有裝過 eLeader 的電腦
才會有這個預設位置；既有有效憑證路徑不會被覆蓋。
...
- **金鑰與憑證**：四欄（API Key、Secret Key、憑證密碼、憑證檔＋「瀏覽…」）。
  ...
  不標（A 只涵蓋金鑰，B 才涵蓋憑證密碼／憑證檔；B 會查憑證到期日，已過期判
  失敗、30 天內到期提醒；伺服器端 `stale` 記錄，重整不會倒回）。
